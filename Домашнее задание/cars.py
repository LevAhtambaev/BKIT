...
async def car_chosen(message: types.Message, state: FSMContext):
    if car_check(message.text.lower()):
...
async def car_eqp__chosen(message: types.Message, state: FSMContext):
    if equipment_check(message.text.lower()):
...
def car_check(text):
    if text not in available_cars_names:
        return 1
    else:
        return 0


def equipment_check(text):
    if text not in available_cars_equipment:
        return 1
    else:
        return 0
